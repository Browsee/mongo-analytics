...
MongoAnalytics.init = function(connection, mongoose) {
  MongoAnalytics.connection = connection;
  MongoAnalytics.dataRow = getModel(connection, mongoose);
};
...
*/
MongoAnalytics.deleteBefore = function(granularity, timestamp) {
  var start = _getTimeBucketWithGranularity(timestamp, granularity);
  return MongoAnalytics.dataRow.remove({time_bucket: {'$lt': start}});
};
